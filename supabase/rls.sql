...
create or replace function public.ttm_is_admin()
returns boolean
...
$$;
revoke all on function public.ttm_is_admin() from public;
grant execute on function public.ttm_is_admin() to authenticated, anon;
...
create policy "admins can read admin registry" on public.ttm_admins
  for select to authenticated using (public.ttm_is_admin());
...
begin
  if to_regclass('public.maps_site_config') is not null then
    ...
    create policy "admins can insert site config" on public.maps_site_config
      for insert to authenticated with check (public.ttm_is_admin());
    ...
    create policy "admins can update site config" on public.maps_site_config
      ...
      using (public.ttm_is_admin()) with check (public.ttm_is_admin());
  end if;
end $$;
...
-- ── cleanup: retire the pre-prefix-convention helper name ───────────────────
-- (earlier revisions shipped is_ttm_admin(); policies above now reference
--  ttm_is_admin(), so the old name can go)

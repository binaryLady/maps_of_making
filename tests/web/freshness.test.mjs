...
// ── Data-contract smoke: the committed snapshot must satisfy the module ─────
test('snapshot: committed demo data computes a marker for every feature', () => {
  const fc = JSON.parse(readFileSync(join(root, 'web', 'demo-data', 'spaces.snapshot.geojson'), 'utf8'));
  ...
  assert.ok(fc.features.length > 2000, 'snapshot has the full seed set');
});

...
async def dispatch(message: Message, session_id: str = "") -> str:
    ...
    try:
        sparql, _, _ = await llm_client.complete_with_system(
            system=NL_TO_SPARQL_SYSTEM.format(ontology_slice=_ONTOLOGY_CACHE or ""),
            user=safe_text,
    ...
    # Format results
    lines = []
    for row in bindings[:15]:
        name = row.get("name", {}).get("value", "")
        url = row.get("url", {}).get("value") or row.get("website", {}).get("value", "")
        if name and url:
            lines.append(f"• [{name}]({url})")
        elif name:
            lines.append(f"• {name}")
        else:
            first_val = next(iter(row.values()), {}).get("value", "")
            lines.append(f"• {first_val}")

    list_text = "\n".join(lines)
    return bernard.nl_result_ack(count=len(bindings), list_text=list_text, sparql_block=sparql)
